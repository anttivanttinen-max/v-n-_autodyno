...
(async () => {
  ...
  const guest=page.locator('#motolabBootGuest');
  if(!await guest.count()) fail('Splash guest fallback missing');
  await guest.click();
  ...
  const menuBox=await page.locator('.mlbm-card').boundingBox();if(!menuBox || menuBox.y<88) fail('User submenu overlaps notification/status area: y='+(menuBox?.y));
  await page.locator('#mlbmClose').click();
  const mic=page.locator('#extMicBtn');if(!await mic.count() || !await mic.isVisible()) fail('Normal-user MIC control is not visible');
  const micState=await page.locator('#extState').textContent();if(/AKTIIVINEN|ACTIVE/i.test(micState||'')) fail('MIC falsely active at browser startup');
  await userNav.click();const liveBtn=page.locator('.mlbm-btn[data-act="live"]');if(!await liveBtn.count()) fail('LIVE menu action missing');await liveBtn.click();await sleep(300);
