...
- New Beta/community/feedback/admin UI must use the same translation layer.
...
- Keep RAW DATA untranslated as the technical export concept.
- Avoid literal Finnish-to-English wording when established automotive/dyno terminology exists.

## Implementation gate
Before next deploy, convert hard-coded visible Finnish strings in active screens/modules into translation keys. Missing English keys must fall back to Finnish rather than blank UI. Dynamic server statuses remain stable machine values and are translated only at presentation. Language switching must not reload or reset active sensor state, run state, user identity or stored data.
