(() => {
'use strict';
const VERSION='motorlab-branding-v2';
...
function visibleBrand(root=document.body){if(!root)return;const w=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);while(w.nextNode()){const n=w.currentNode,p=n.parentElement;if(!p||['SCRIPT','STYLE','CODE','PRE','TEXTAREA'].includes(p.tagName))continue;const old=n.nodeValue||'';if(!/motolab/i.test(old))continue;const next=old.replace(/VÄNÄ\s+MOTOLAB/gi,'VÄNÄ MOTORLAB').replace(/VäNä\s+MotoLab/g,'VäNä MotorLab').replace(/MotoLab/g,'MotorLab').replace(/MOTOLAB/g,'MOTORLAB');if(next!==old)n.nodeValue=next}}
function relabel(){const title=`VäNä MotorLab ${globalThis.MOTOLAB_RELEASE?.label||''}`.trim();if(document.title!==title)document.title=title;document.querySelectorAll('.brand').forEach(b=>{if(b.getAttribute('aria-label')!=='VäNä MotorLab')b.setAttribute('aria-label','VäNä MotorLab')});const fv=$('motolabFullVersion'),txt=`VÄNÄ MOTORLAB ${globalThis.MOTOLAB_RELEASE?.label||''} • build ${globalThis.MOTOLAB_RELEASE?.build||''}`;if(fv&&fv.textContent!==txt)fv.textContent=txt;visibleBrand()}
function splash(){if($('motorlabSplash'))return;const now=Date.now(),prev=Number(sessionStorage.getItem('motorlab_splash_seen_at')||0);if(now-prev<30000)return;sessionStorage.setItem('motorlab_splash_seen_at',String(now));const el=document.createElement('div');el.id='motorlabSplash';el.innerHTML=`<img src="${ASSET}" alt="VäNä Motorsport MotorLab"><div id="motorlabSplashVersion">${globalThis.MOTOLAB_RELEASE?.label||''}</div>`;document.body.classList.add('ml-splash-open');document.body.appendChild(el);const born=performance.now(),close=()=>{const wait=Math.max(0,1100-(performance.now()-born));setTimeout(()=>{if(!el.isConnected)return;el.classList.add('ml-hide');setTimeout(()=>{el.remove();document.body.classList.remove('ml-splash-open')},380)},wait)};if(document.readyState==='complete')close();else window.addEventListener('load',close,{once:true});setTimeout(close,1800)}
function boot(){style();relabel();splash();let pending=false;new MutationObserver(records=>{if(pending||!records.some(r=>r.addedNodes?.length))return;pending=true;requestAnimationFrame(()=>{pending=false;relabel()})}).observe(document.documentElement,{subtree:true,childList:true})}
