...
function bootstrapState(){const db=readJson(USERS_FILE,{schema:'motolab_users_v1',users:[]});const admin=(db.users||[]).find(u=>u.role==='admin');return {db,admin,armed:!db.ownerBootstrapConsumedAt&&!admin,consumed:!!db.ownerBootstrapConsumedAt,adminExists:!!admin,recoveryUsed:!!db.ownerBootstrapRecoveryConsumedAt}}
function tokenFor(u,deviceId){const t=Date.now(),exp=t+365*86400000;return {token:sign({v:2,kind:'device',userId:u.userId,deviceId,iat:t,exp}),expiresAt:new Date(exp).toISOString()}}
...
async function tryBootstrap(req,res,origin){
 ...
 const s=bootstrapState(),db=s.db,now=new Date().toISOString();

 if(s.consumed&&s.adminExists){
   if(s.recoveryUsed)return send(res,410,{ok:false,error:'Owner recovery already used'},origin);
   const u=s.admin;
   ...
   if(!d){d={deviceId,label,createdAt:now,lastSeenAt:now};u.devices.push(d)}else{d.lastSeenAt=now;if(label)d.label=label}
   u.ownerRecoveryAt=now;u.ownerRecoveryDeviceId=deviceId;u.ownerClaimedDeviceId=deviceId;
   db.ownerBootstrapRecoveryConsumedAt=now;db.ownerBootstrapRecoveryConsumedByDeviceId=deviceId;db.updatedAt=now;
   atomic(USERS_FILE,JSON.stringify(db,null,2));
   return send(res,200,{ok:true,...tokenFor(u,deviceId),deviceId,user:publicUser(u),status:'active',owner:true,recovered:true},origin);
 }

 if(s.consumed)return send(res,410,{ok:false,error:'Owner bootstrap already used'},origin);
 if(s.adminExists)return send(res,409,{ok:false,error:'Owner already exists'},origin);
 let u=(db.users||[]).find(x=>(x.devices||[]).some(d=>d.deviceId===deviceId));
 if(!u){u={userId:id('usr'),nickname:OWNER_NICKNAME,status:'active',role:'admin',invitedBy:null,inviteId:'owner_bootstrap_once',createdAt:now,approvedAt:now,devices:[{deviceId,label,createdAt:now,lastSeenAt:now}]};db.users.push(u)}
 else{u.nickname=OWNER_NICKNAME;u.status='active';u.role='admin';u.approvedAt=u.approvedAt||now;const d=(u.devices||[]).find(x=>x.deviceId===deviceId);if(d){d.lastSeenAt=now;if(label)d.label=label}}
 ...
 atomic(USERS_FILE,JSON.stringify(db,null,2));
 return send(res,200,{ok:true,...tokenFor(u,deviceId),deviceId,user:publicUser(u),status:'active',owner:true},origin);
}
...
http.createServer=function(listener){return originalCreateServer(async(req,res)=>{
 ...
 if(req.method==='GET'&&u.pathname==='/api/users/v1/owner-bootstrap-status'){const s=bootstrapState();return send(res,200,{ok:true,armed:s.armed,consumed:s.consumed,adminExists:s.adminExists,recoveryUsed:s.recoveryUsed},origin)}
 if(req.method==='POST'&&u.pathname==='/api/beta/v1/activate'){const handled=await tryBootstrap(req,res,origin);if(handled!==false)return handled}
 return listener(req,res)
})};
console.log('MotoLab one-time owner bootstrap: armed by hash; one post-bootstrap owner recovery allowed');
