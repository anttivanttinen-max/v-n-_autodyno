...
const {createVehicleWebSearch}=require('./vehicle_web_search');
const {createVehicleRequestQueue}=require('./vehicle_request_queue');
...
const vehicleWebSearch=createVehicleWebSearch({dataDir:DATA_DIR});
const vehicleRequests=createVehicleRequestQueue({dataDir:DATA_DIR});
...
async function handleRawPost(req,res,u,origin){
 ...
 const queuedVehicleRequests=vehicleRequests.captureChunk(env.chunk,{deviceId:env.deviceId||device,sessionId:env.chunk.sessionId||null});
 return json(res,201,{ok:true,id:env.chunk.id,deviceId:env.deviceId,sessionId:env.chunk.sessionId||null,queuedVehicleRequests},origin)
}
...
async function handleVehicleResearchPost(req,res,u,origin){
 if(!authRead(req,u))return json(res,401,{ok:false,error:'Invalid read key'},origin);
 let body;try{body=await readBody(req)}catch(e){return json(res,e.status||400,{ok:false,error:e.message},origin)}
 let env;try{env=parseJson(body)}catch{return json(res,400,{ok:false,error:'Invalid JSON'},origin)}
 try{const result=await vehicleWebSearch.research(env?.query);vehicleRequests.update(env?.query,{status:result.status==='found'?'researched':'not_found',lastResultAt:new Date().toISOString(),lastError:null});return json(res,200,{ok:true,result},origin)}catch(e){if(env?.query)vehicleRequests.update(env.query,{status:e.code==='SEARCH_DISABLED'?'pending':'error',lastError:e.message});return json(res,e.status||500,{ok:false,error:e.message,code:e.code||null,search:vehicleWebSearch.status()},origin)}
}
async function handleVehicleResearchPending(req,res,u,origin){
 if(!authRead(req,u))return json(res,401,{ok:false,error:'Invalid read key'},origin);
 let body={};try{const b=await readBody(req);if(b.length)body=parseJson(b)}catch(e){return json(res,400,{ok:false,error:e.message},origin)}
 const limit=Math.max(1,Math.min(10,Number(body?.limit||1))),items=vehicleRequests.pending(limit),results=[];
 if(!vehicleWebSearch.status().enabled)return json(res,503,{ok:false,error:'Vehicle web search is prepared but not enabled',code:'SEARCH_DISABLED',pending:items.length,search:vehicleWebSearch.status()},origin);
 for(const item of items){try{const result=await vehicleWebSearch.research(item.query);vehicleRequests.update(item.query,{status:result.status==='found'?'researched':'not_found',lastResultAt:new Date().toISOString(),lastError:null});results.push({query:item.query,ok:true,status:result.status,candidates:result.candidates?.length||0})}catch(e){vehicleRequests.update(item.query,{status:'error',lastError:e.message});results.push({query:item.query,ok:false,error:e.message});break}}
 return json(res,200,{ok:true,processed:results.length,results,remaining:vehicleRequests.pending(50).length},origin)
}
...
function handleRead(req,res,u,origin,headOnly=false){
 if(u.pathname==='/health')return json(res,200,{ok:true,service:'vana-motolab-raw-sync',researchSync:true,vehicleWebSearch:vehicleWebSearch.status(),vehiclePending:vehicleRequests.pending(50).length,githubMirror:mirror.status(),time:new Date().toISOString()},origin,headOnly);
 if(headOnly)return json(res,404,{ok:false,error:'Not found'},origin,true);
 if(u.pathname==='/api/vehicle/v1/search-status')return json(res,200,{ok:true,search:vehicleWebSearch.status(),pending:vehicleRequests.pending(50).length},origin);
 if(u.pathname==='/api/vehicle/v1/requests'){if(!authRead(req,u))return json(res,401,{ok:false,error:'Invalid read key'},origin);return json(res,200,{ok:true,items:vehicleRequests.list()},origin)}
...
const server=http.createServer(async(req,res)=>{const origin=String(req.headers.origin||'');if(req.method==='OPTIONS'){if(origin&&origin!==ALLOWED_ORIGIN){res.writeHead(403);return res.end()}res.writeHead(204,corsHeaders(origin));return res.end()}const u=parseUrl(req);try{if(req.method==='POST'&&u.pathname==='/api/raw/v1/chunk')return await handleRawPost(req,res,u,origin);if(req.method==='POST'&&u.pathname.startsWith('/api/research/v1/'))return await handleResearchPost(req,res,u,origin);if(req.method==='POST'&&u.pathname==='/api/vehicle/v1/research')return await handleVehicleResearchPost(req,res,u,origin);if(req.method==='POST'&&u.pathname==='/api/vehicle/v1/research-pending')return await handleVehicleResearchPending(req,res,u,origin);if(req.method==='GET'||req.method==='HEAD')return handleRead(req,res,u,origin,req.method==='HEAD');return json(res,405,{ok:false,error:'Method not allowed'},origin)}catch(e){console.error(e);return json(res,500,{ok:false,error:'Internal server error'},origin)}});
server.listen(PORT,'0.0.0.0',()=>{console.log(`MotoLab sync listening on :${PORT}, data=${DATA_DIR}, vehicleSearch=${vehicleWebSearch.status().enabled?'ON':'OFF'}`);mirror.backfill().then(x=>{if(x.enabled)console.log(`MotoLab GitHub mirror backfill queued ${x.queued} files`);else console.log('MotoLab GitHub mirror disabled')}).catch(e=>console.error('MotoLab mirror backfill:',e))});
