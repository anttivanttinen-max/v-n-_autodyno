...
async function handleVehicleResearchPost(req,res,u,origin){
 ...
 let body;try{body=await readBody(req)}catch(e){return json(res,e.status||400,{ok:false,error:e.message},origin)}
 let env;try{env=parseJson(body)}catch{return json(res,400,{ok:false,error:'Invalid JSON'},origin)}
 try{const result=await vehicleWebSearch.research(env?.query);return json(res,200,{ok:true,result},origin)}catch(e){return json(res,e.status||500,{ok:false,error:e.message,code:e.code||null,search:vehicleWebSearch.status()},origin)}
}
...
function handleRead(req,res,u,origin,headOnly=false){
 if(u.pathname==='/health')return json(res,200,{ok:true,service:'vana-motolab-raw-sync',researchSync:true,vehicleWebSearch:vehicleWebSearch.status(),githubMirror:mirror.status(),time:new Date().toISOString()},origin,headOnly);
 if(headOnly)return json(res,404,{ok:false,error:'Not found'},origin,true);
 if(u.pathname==='/api/vehicle/v1/search-status')return json(res,200,{ok:true,search:vehicleWebSearch.status()},origin);
