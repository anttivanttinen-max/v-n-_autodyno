...
function corsHeaders(origin){const allow=origin===ALLOWED_ORIGIN?origin:'';return {...(allow?{'Access-Control-Allow-Origin':allow}:{}),'Vary':'Origin','Access-Control-Allow-Methods':'GET,POST,OPTIONS','Access-Control-Allow-Headers':'Content-Type,X-MotoLab-Ingest-Key,X-MotoLab-Read-Key','Access-Control-Max-Age':'86400'}}
function json(res,status,obj,origin){const body=Buffer.from(JSON.stringify(obj));res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Content-Length':body.length,'Cache-Control':'no-store',...(origin?corsHeaders(origin):{})});res.end(body)}
...
function handleRead(req,res,u,origin){
 if(u.pathname==='/health')return json(res,200,{ok:true,service:'vana-motolab-raw-sync',researchSync:true,time:new Date().toISOString()},origin);
