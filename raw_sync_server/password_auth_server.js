...
function nicknameKey(v){return String(v||'').trim().toLocaleLowerCase('fi-FI')}
...
http.createServer=function(listener){return originalCreateServer(async(req,res)=>{const origin=String(req.headers.origin||'');let u;try{u=new URL(req.url,'http://localhost')}catch{return listener(req,res)}
 if(req.method==='OPTIONS'&&origin===ALLOWED_ORIGIN&&(u.pathname==='/api/users/v1/password'||u.pathname==='/api/users/v1/password-login')){res.writeHead(204,{'Access-Control-Allow-Origin':origin,'Vary':'Origin','Access-Control-Allow-Methods':'POST,OPTIONS','Access-Control-Allow-Headers':'Content-Type,X-MotoLab-Beta-Token','Access-Control-Max-Age':'86400'});return res.end()}
...
console.log('MotoLab password auth: enabled (scrypt-v1)');
