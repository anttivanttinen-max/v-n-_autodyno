(() => {
'use strict';
const MODULE_VERSION='v32-raw-auto-sync-1';
...
async function syncNow(force=false){
 ...
 try{
  ...
  for(const item of [...q.pending]){
   ...
   try{
    await sendChunk(chunk,cfg);q.pending=q.pending.filter(x=>x.id!==item.id);q.sent=(q.sent||0)+1;q.lastOk=Date.now();q.lastError=null;done++;writeQueue(q)
   }catch(e){
    const live=q.pending.find(x=>x.id===item.id);if(live){live.tries=(live.tries||0)+1;live.nextAttempt=Date.now()+Math.min(15*60*1000,5000*Math.pow(2,Math.min(7,live.tries-1)))}q.lastError=String(e.message||e);writeQueue(q);statusText('Synkkaus jäi jonoon: '+q.lastError);break
   }
  }
