...
async function restoreSensors(){const p=readPrefs();restoring=true;try{if(p.gps&&!$('gpsDot')?.classList.contains('on'))try{if(typeof startGPS==='function')await startGPS()}catch{}if(p.imu&&!$('imuDot')?.classList.contains('on')){const iosNeedsGesture=typeof DeviceMotionEvent!=='undefined'&&typeof DeviceMotionEvent.requestPermission==='function';if(!iosNeedsGesture)try{if(typeof startIMU==='function')await startIMU()}catch{}else logRecovery('imu_restore_waiting_for_user_gesture',{reason:'ios_motion_permission'})}/* iOS may require a user gesture for a fresh getUserMedia start. */}finally{restoring=false;decorateMicState()}}
