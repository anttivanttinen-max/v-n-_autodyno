(() => {
'use strict';
const MODULE='trip-gear-guard-v8-all-gears-popup';
const TARGET_GEAR=3;
...
function publish(ev){const payload={module:MODULE,state,gear3:state==='third',learningAllowed:true,micDataAllowed:true,t:Date.now(),evidence:ev||null,manualGear};globalThis.MOTOLAB_TRIP_GEAR_GUARD=payload;try{globalThis.dispatchEvent(new CustomEvent('motolab-trip-gear-guard',{detail:payload}))}catch{}}
function suspectedGear(ev){if(ev?.kind==='gear'&&[1,2,3,4,5,6].includes(+ev.detectedGear))return +ev.detectedGear;if(ev?.kind==='manual'&&[1,2,3,4,5,6].includes(+ev.detectedGear))return +ev.detectedGear;if(state==='third')return 3;return null}
...
function promptGear(sus,ev,force=false){
 if(![1,2,3,4,5,6].includes(+sus))return;if(!force&&!researchActive())return;const key=`${state}:${sus}`;if(!force&&(promptOpen||(key===lastPromptKey&&Date.now()-lastPromptAt<5000)))return;
 styles();closePrompt();promptOpen=true;lastPromptKey=key;lastPromptAt=Date.now();const o=document.createElement('div');o.id='tripGearPrompt';o.setAttribute('role','dialog');o.setAttribute('aria-modal','true');o.innerHTML=`<div class="tgp-card"><h2>VAIHTEEN VAIHTO HAVAITTU</h2><p>Sovellus epäilee <b>${sus}. vaihdetta</b>. Vahvista oikea vaihde.</p><div class="tgp-grid">${[1,2,3,4,5,6].map(g=>`<button type="button" class="tgp-btn ${g===sus?'suspect':''}" data-gear="${g}">${g}</button>`).join('')}</div><div class="tgp-note">HARMAA = vaihtoehto • PUNAINEN = sovelluksen epäily • VIHREÄ = vahvistettu</div><button class="tgp-close" type="button">JATKA ILMAN VAHVISTUSTA</button></div>`;document.body.appendChild(o);
...
function confirmGear(gear){
 gear=+gear;if(![1,2,3,4,5,6].includes(gear))return;manualGear=gear;manualGearUntil=Date.now()+MANUAL_HOLD_MS;const next=gear===3?'third':'other';const t=Date.now();globalThis.MOTOLAB_CONFIRMED_GEAR={gear,t,origin:'user_confirmed'};commit(next,{kind:'manual',isThird:gear===3,detectedGear:gear,confirmedAt:t});try{if(typeof addLearningEvent==='function')addLearningEvent('trip_gear_manual_confirm',{module:MODULE,gear})}catch{}try{globalThis.dispatchEvent(new CustomEvent('motolab-gear-confirmed',{detail:{gear,t,origin:'user_confirmed',module:MODULE}}))}catch{}paint()
}
function ensureUi(){
 const panel=$g('tripResearchPanel');if(!panel||$g('tripGearGuardBox'))return !!panel;const box=document.createElement('div');box.id='tripGearGuardBox';box.className='statusbox';box.style.marginTop='8px';box.innerHTML=`<div style="font-weight:900;margin-bottom:5px">VAIHDEVAHTI</div><div id="tripGearGuardStatus">Vaihteen tunnistus odottaa tutkimusajoa.</div><div style="margin-top:5px;color:#8f969f;font-size:8px">Kaikki vaihteet tallennetaan. Epäselvä vaihde voidaan vahvistaa keskeltä näyttöä.</div>`;const st=$g('tripResearchStatus');if(st)st.insertAdjacentElement('beforebegin',box);else panel.appendChild(box);return true
}
...
function patchLearning(){
 try{if(typeof collectLearningSample!=='function'||collectLearningSample.__tripGearGuardPatched)return false;const orig=collectLearningSample;const wrapped=function(s){const before=Array.isArray(learningBuffer)?learningBuffer.length:0,out=orig(s);try{if(researchActive()&&Array.isArray(learningBuffer)&&learningBuffer.length>before){const row=learningBuffer[learningBuffer.length-1],g=globalThis.MOTOLAB_TRIP_GEAR_GUARD||{};row.tripGearGuardState=g.state||'unknown';row.tripGear3Confirmed=g.state==='third';row.tripGearLearningAllowed=true;row.tripMicDataAllowed=true;row.tripConfirmedGear=manualGear||null;row.tripDetectedGear=g.evidence?.detectedGear||null;row.tripAllGearsLearning=true}}catch{}return out};wrapped.__tripGearGuardPatched=true;collectLearningSample=wrapped;return true}catch{return false}
}
