(() => {
'use strict';
const MODULE='trip-gear-guard-v2';
const TARGET_GEAR=3;
...
function ensureUi(){
 ...
 const box=document.createElement('div');box.id='tripGearGuardBox';box.className='statusbox';box.style.marginTop='8px';box.innerHTML=`<button id="tripGear3Confirm" class="action full" type="button" style="margin-bottom:7px">3. VAIHDE SISÄSSÄ</button><div id="tripGearGuardStatus">3. vaihteen valvonta odottaa tutkimusajoa.</div>`;
...
function paint(ev){
 ...
 if(researchActive()){
   if(state==='third')text='✅ 3. VAIHDE • mikin opetus käytössä';
   else if(state==='other')text='⚠️ VAIHDE VAIHTUI • mikin opetus tauolla • RAW tallentuu';
   else text='… TUNNISTETAAN VAIHDETTA • mikin opetus odottaa • RAW tallentuu';
   const e=ev||evidence();
 ...
 if(b){b.classList.toggle('on',state==='third');b.textContent=state==='third'?'✅ 3. VAIHDE SISÄSSÄ':'3. VAIHDE SISÄSSÄ'}
