...
const MODULES=[
 'splash_boot','diagnostics','vehicle_catalog_finland_v2_loader','vehicle_lookup','technical_specs','maintenance',
...
function inject(html){
 ...
 const tags=[];for(const n of MODULES){if(!new RegExp(`(?:\\/|\\.)${n}\\.js(?:[?"'])`).test(html))tags.push(`<script src="./${n}.js?v=${encodeURIComponent(VERSION)}&build=${encodeURIComponent(BUILD)}"></script>`)}
...
async function freshIndex(){const r=await fetch(bust('index.html'),{cache:'no-store'});if(!r.ok)throw Error('index '+r.status);return new Response(inject(await r.text()),{headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store, no-cache, must-revalidate'}})}
async function networkFirst(req){try{const r=await fetch(new Request(req,{cache:'reload'}));if(r.ok){const c=await caches.open(CACHE);c.put(req,r.clone()).catch(()=>{})}return r}catch(e){const hit=await caches.match(req);if(hit)return hit;throw e}}
self.addEventListener('message',e=>{if(e.data?.type==='SKIP_WAITING')self.skipWaiting()});
self.addEventListener('notificationclick',e=>{e.notification.close();const target=e.notification?.data?.url||'./?adminUsers=1';e.waitUntil((async()=>{const list=await self.clients.matchAll({type:'window',includeUncontrolled:true});for(const client of list){try{if('navigate'in client)await client.navigate(target);await client.focus();return}catch{}}if(self.clients.openWindow)await self.clients.openWindow(target)})())});
...
self.addEventListener('activate',e=>e.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>k!==CACHE&&(k.startsWith('vana-motolab-')||k.startsWith('vana-motorlab-'))).map(k=>caches.delete(k)));await self.clients.claim();const clients=await self.clients.matchAll({type:'window',includeUncontrolled:true});for(const client of clients)client.postMessage({type:'MOTOLAB_SW_ACTIVE',build:BUILD})})()));
self.addEventListener('fetch',e=>{if(e.request.method!=='GET')return;const u=new URL(e.request.url);if(u.origin!==location.origin)return;if(e.request.mode==='navigate'){e.respondWith(freshIndex().then(async r=>{const c=await caches.open(CACHE);await c.put(bust('index.html'),r.clone());return r}).catch(()=>caches.match(bust('index.html'))));return}if(/\.(?:js|json|webmanifest|png|webp)$/.test(u.pathname)){e.respondWith(networkFirst(e.request));return}e.respondWith(caches.match(e.request).then(hit=>hit||fetch(e.request)))});
