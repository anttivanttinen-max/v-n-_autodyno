importScripts("./version.js?build=2026-08-16f");
...
function injectModules(html){
 const scripts=[];
 if(!html.includes("vehicle_lookup.js"))scripts.push('<script src="./vehicle_lookup.js?v='+V+'"></script>');
 if(!html.includes("technical_specs.js"))scripts.push('<script src="./technical_specs.js?v='+V+'"></script>');
 if(!html.includes("maintenance.js"))scripts.push('<script src="./maintenance.js?v='+V+'"></script>');
 if(!html.includes("gps_master_learning.js"))scripts.push('<script src="./gps_master_learning.js?v='+V+'"></script>');
 if(!html.includes("raw_sync.js"))scripts.push('<script src="./raw_sync.js?v='+V+'"></script>');
 ...
 html=html.replace(/<link rel="manifest" href="manifest\.webmanifest(?:\?[^\"]*)?">/,'<link rel="manifest" href="manifest.webmanifest?v='+V+'">');
...
async function cacheInjectedIndex(cache){const r=await fetch("./index.html?v="+V,{cache:"no-store"});if(!r.ok)throw new Error("index "+r.status);const html=injectModules(await r.text());await cache.put("./index.html?v="+V,new Response(html,{headers:{"Content-Type":"text/html; charset=utf-8","Cache-Control":"no-cache, no-store, must-revalidate"}}));}
