...
async function countResearchStore(db,name){if(!db.objectStoreNames.contains(name))return 0;return researchRequest(db.transaction(name,'readonly').objectStore(name).count())}
async function recentResearchKeys(db,name,limit=8){if(!db.objectStoreNames.contains(name))return[];return new Promise((resolve,reject)=>{const out=[],store=db.transaction(name,'readonly').objectStore(name);let req;try{req=store.openKeyCursor(null,'prev')}catch{req=store.openCursor(null,'prev')}req.onerror=()=>reject(req.error);req.onsuccess=()=>{const c=req.result;if(!c||out.length>=limit){resolve(out);return}out.push(String(c.primaryKey??c.key??c.value?.id??''));c.continue()}})}
async function researchKeys(db,name){if(!db.objectStoreNames.contains(name))return[];return new Promise((resolve,reject)=>{const out=[],store=db.transaction(name,'readonly').objectStore(name);let req;try{req=store.openKeyCursor()}catch{req=store.openCursor()}req.onerror=()=>reject(req.error);req.onsuccess=()=>{const c=req.result;if(!c){resolve(out);return}out.push(String(c.primaryKey??c.key??c.value?.id??''));c.continue()}})}
async function researchRow(db,name,id){if(!db.objectStoreNames.contains(name))return null;return researchRequest(db.transaction(name,'readonly').objectStore(name).get(id))}
async function researchRawSummary(){let db;try{db=await openResearchDb();const done=[...researchSent()],[sessions,timelineChunks]=await Promise.all([countResearchStore(db,'sessions'),countResearchStore(db,'timelineChunks')]);return {sessions,timelineChunks,sessionsMarkedSent:Math.min(sessions,done.filter(x=>x.startsWith('sessions:')).length),timelineChunksMarkedSent:Math.min(timelineChunks,done.filter(x=>x.startsWith('timelineChunks:')).length)}}finally{db?.close()}}
...
function rawDataStyles(){if($('mlRawDataStyle'))return;const s=document.createElement('style');s.id='mlRawDataStyle';s.textContent='.ml-raw-data-overlay{position:fixed;inset:0;z-index:320000;background:#000e;display:flex;align-items:flex-end;justify-content:center;padding:12px;color:#fff;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif}.ml-raw-data-card{width:min(100%,480px);max-height:88dvh;overflow:auto;border:1px solid #ff263f;border-radius:20px;background:#090b0e;padding:15px}.ml-raw-data-card h2{margin:0 0 6px;font-size:17px}.ml-raw-data-copy{font-size:10px;line-height:1.45;color:#bdc2ca;white-space:pre-line}.ml-raw-data-list{margin-top:10px;border-top:1px solid #30343b}.ml-raw-data-row{padding:8px 0;border-bottom:1px solid #30343b;font-size:9px;line-height:1.35}.ml-raw-data-row b{display:block;color:#fff;overflow-wrap:anywhere}.ml-raw-data-action{width:100%;min-height:45px;margin-top:9px;border:1px solid #ff263f;border-radius:11px;background:#a80d20;color:#fff;font-weight:1000}.ml-raw-data-close{background:#15181d;border-color:#555}' ;document.head.appendChild(s)}
async function openResearchRawData(){rawDataStyles();$('mlRawDataPanel')?.remove();const panel=document.createElement('div');panel.id='mlRawDataPanel';panel.className='ml-raw-data-overlay';panel.innerHTML='<div class="ml-raw-data-card"><h2>OMA RAW-DATA</h2><div id="mlRawDataStatus" class="ml-raw-data-copy">Näkymä valmis. RAW-payloadia ei ladata tähän ruutuun.</div><div id="mlRawDataList" class="ml-raw-data-list"><div class="ml-raw-data-row">Luetaan vain viimeisimmät tunnisteet…</div></div><button id="mlRawDataSend" class="ml-raw-data-action" type="button">LÄHETÄ RAW-DATA NYT</button><button id="mlRawDataClose" class="ml-raw-data-action ml-raw-data-close" type="button">SULJE</button></div>';document.body.appendChild(panel);const status=$('mlRawDataStatus'),list=$('mlRawDataList'),send=$('mlRawDataSend');const refresh=async()=>{const summary=await researchRawSummary();if(!panel.isConnected)return;status.textContent=rawRecoveryText(summary)+(!summary.sessions&&!summary.timelineChunks?'\n\nEI PAIKALLISTA RAW-DATAA TÄLLÄ PUHELIMELLA.':'');let db;try{db=await openResearchDb();const done=researchSent(),[sessions,timeline]=await Promise.all([recentResearchKeys(db,'sessions',8),recentResearchKeys(db,'timelineChunks',8)]);if(!panel.isConnected)return;list.innerHTML=[...sessions.map(id=>`<div class="ml-raw-data-row"><b>SESSION • ${id||'–'}</b>${done.has(`sessions:${id}`)?'lähetetty':'odottaa lähetystä'}</div>`),...timeline.map(id=>`<div class="ml-raw-data-row"><b>TIMELINE • ${id||'–'}</b>${done.has(`timelineChunks:${id}`)?'lähetetty':'odottaa lähetystä'}</div>`)].join('')||'<div class="ml-raw-data-row">Ei paikallisia tunnisteita.</div>'}finally{db?.close()}};$('mlRawDataClose').onclick=()=>panel.remove();send.onclick=async()=>{send.disabled=true;status.textContent='RAW-dataa lähetetään nyt…';try{const report=await globalThis.MotoLabV34RuntimeFixes.syncResearchRaw();status.textContent=`${rawRecoveryText(report.after)}\n\nTällä ajolla lähetetty: sessions ${report.sentThisRun.sessions} • timelineChunks ${report.sentThisRun.timelineChunks}${report.error?`\n\nVIRHE: ${report.error}`:'\n\nLÄHETYS VALMIS.'}`;await refresh()}catch(e){status.textContent='VIRHE: '+(e?.message||e)}finally{send.disabled=false}};setTimeout(()=>refresh().catch(e=>{if(panel.isConnected)status.textContent='VIRHE: '+(e?.message||e)}),0);return panel}
...
async function syncResearchRaw(){
 ...
 researchSyncBusy=true;let db;
 try{
  db=await openResearchDb();const done=researchSent();
  for(const store of ['sessions','timelineChunks']){
   const ids=await researchKeys(db,store);
   for(const id of ids){const key=`${store}:${id}`;if(done.has(key))continue;const row=await researchRow(db,store,id);if(!row)continue;await sendResearchRow(store,row);done.add(key);saveResearchSent(done);await new Promise(r=>setTimeout(r,0))}
  }
...
globalThis.MotoLabV34RuntimeFixes={version:VERSION,patchSaveRun,ensureGearEditor,confirmedGear,syncResearchRaw,openResearchRawData};
