...
async function openResearchRawData(){rawDataStyles();const old=$('mlRawDataPanel');old?.remove();const panel=document.createElement('div');panel.id='mlRawDataPanel';panel.className='ml-raw-data-overlay';panel.innerHTML='<div class="ml-raw-data-card"><h2>OMA RAW-DATA</h2><div id="mlRawDataStatus" class="ml-raw-data-copy">Luetaan puhelimen RAW-dataa…</div><div id="mlRawDataList" class="ml-raw-data-list"></div><button id="mlRawDataSend" class="ml-raw-data-action" type="button">LÄHETÄ RAW-DATA NYT</button><button id="mlRawDataClose" class="ml-raw-data-action ml-raw-data-close" type="button">SULJE</button></div>';document.body.appendChild(panel);const status=$('mlRawDataStatus'),list=$('mlRawDataList'),send=$('mlRawDataSend');const refresh=async()=>{const summary=await researchRawSummary();status.textContent=rawRecoveryText(summary)+(!summary.sessions&&!summary.timelineChunks?'\n\nEI PAIKALLISTA RAW-DATAA TÄLLÄ PUHELIMELLA.':'');let db;try{db=await openResearchDb();const done=researchSent(),sessions=(await readResearchStore(db,'sessions')).slice(-8).reverse(),timeline=(await readResearchStore(db,'timelineChunks')).slice(-8).reverse();list.innerHTML=[...sessions.map(x=>`<div class="ml-raw-data-row"><b>SESSION • ${String(x.id||'–')}</b>${x.startedAt||x.createdAt||'aika puuttuu'} • ${done.has(`sessions:${x.id}`)?'lähetetty':'odottaa lähetystä'}</div>`),...timeline.map(x=>`<div class="ml-raw-data-row"><b>TIMELINE • ${String(x.id||'–')}</b>${x.createdAt||'aika puuttuu'} • ${(x.frames||[]).length} kehystä • ${done.has(`timelineChunks:${x.id}`)?'lähetetty':'odottaa lähetystä'}</div>`)].join('')||'<div class="ml-raw-data-row">Ei näytettäviä paikallisia rivejä.</div>'}finally{db?.close()}};send.onclick=async()=>{send.disabled=true;status.textContent='RAW-dataa lähetetään nyt…';try{const report=await globalThis.MotoLabV34RuntimeFixes.syncResearchRaw();status.textContent=`${rawRecoveryText(report.after)}\n\nTällä ajolla lähetetty: sessions ${report.sentThisRun.sessions} • timelineChunks ${report.sentThisRun.timelineChunks}${report.error?`\n\nVIRHE: ${report.error}`:'\n\nLÄHETYS VALMIS.'}`;await refresh()}catch(e){status.textContent='VIRHE: '+(e?.message||e)}finally{send.disabled=false}};$('mlRawDataClose').onclick=()=>panel.remove();refresh().catch(e=>{status.textContent='VIRHE: '+(e?.message||e)});return panel}
