(() => {
'use strict';
const VERSION='motolab-splash-boot-v3-account-required';
...
function setBusy(on){for(const id of ['motolabBootActivate','motolabBootRefresh']){const b=$(id);if(b)b.disabled=!!on}}
function pendingInvite(){try{return sessionStorage.getItem('motorlab_pending_invite')||new URLSearchParams(location.search).get('invite')||''}catch{return ''}}
function loginMarkup(){return `<div class="mls-login" id="motolabBootLogin"><div class="mls-login-title">KIRJAUDU / REKISTERÖIDY</div><div class="mls-login-copy">Kirjaudu nimimerkillä ja salasanalla. Uusi käyttäjä voi rekisteröityä ja odottaa ylläpidon hyväksyntää.</div><label>Nimimerkki</label><input id="motolabBootNick" maxlength="32" autocomplete="nickname" value="${esc(localStorage.getItem('motolab_user_nickname')||'')}"><label>Kutsukoodi</label><input id="motolabBootCode" autocomplete="one-time-code" value="${esc(pendingInvite())}" placeholder="Kutsukoodi"><button id="motolabBootActivate" class="mls-primary">AKTIVOI TÄMÄ LAITE</button><button id="motolabBootRefresh" class="mls-secondary">PÄIVITÄ TILA</button><div class="mls-msg" id="motolabBootLoginMsg"></div></div>`}
function showLogin(message=''){if(finished||loginShown)return;loginShown=true;const slot=$('motolabBootPanel');if(!slot)return;slot.innerHTML=loginMarkup();slot.classList.add('mls-panel-on');const msg=$('motolabBootLoginMsg');if(msg)msg.textContent=message|| (pendingInvite()?'Kutsukoodi täytetty linkistä. Valitse nimimerkki ja aktivoi laite.':'');const act=$('motolabBootActivate'),ref=$('motolabBootRefresh');
...
function boot(){const r=root();if(!r)return;status('LADATAAN MOTORLABIA…');const img=$('motolabBootImage');if(img){if(img.complete&&img.naturalWidth>0)r.classList.add('mls-image-ok');img.addEventListener('load',()=>r.classList.add('mls-image-ok'),{once:true});img.addEventListener('error',()=>{r.classList.add('mls-image-fallback');status('MOTORLAB')},{once:true})}
 ...
 setTimeout(()=>{if(!finished)showLogin('Kirjaudu tai rekisteröidy jatkaaksesi käyttäjätilillä.')},FAILSAFE_MS);
}
