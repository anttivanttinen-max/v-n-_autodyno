(() => {
'use strict';
const MODULE='motolab-admin-test-tools-v1';
...
function ensureGearUi(){
 if(!isAdmin()||!researchActive()){$('motolabGearTestFloat')?.remove();return false}
...
async function confirmGear(gear){
 ...
 const detail={gear,at:Date.now(),suspectedGear:detectedOtherGear(g),guardState:g.state||'unknown',evidence:g.evidence||null,source:'admin_manual_reference'};
