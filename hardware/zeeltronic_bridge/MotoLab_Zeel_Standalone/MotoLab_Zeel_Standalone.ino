...
static const char *FW = "motolab-zeel-standalone-v3.0";
static const char *BLE_NAME = "MotoLab-Zeel";
static const char *WIFI_NAME = "MotoLab-Zeel";
static const char *WIFI_PASSWORD = "motolab10";
...
static const size_t ZEEL_IMAGE_SIZE = 480;

static WebServer web(80);
static uint8_t zeelImage[ZEEL_IMAGE_SIZE];
static uint8_t protocolRx[1024];
static volatile size_t protocolRxHead = 0;
static volatile size_t protocolRxTail = 0;
static volatile bool protocolReadActive = false;
static volatile bool protocolReadBusy = false;
static bool haveZeelImage = false;
static String lastReadResult = "not_read";
...
// Hard invariant for standalone v3: arbitrary writes are never enabled.
static constexpr bool writeUnlocked = false;
...
// The standalone reader may transmit only the three captured Read selectors
// and the 0x64 polling byte. This path is intentionally not exposed over BLE
// or HTTP and cannot carry arbitrary/programming data.
static bool sendReadProtocolBytes(const uint8_t *data, size_t length) {
  if (!protocolReadActive || !ftdiReady || !epOut || !deviceHandle || !length) return false;
  const bool selector = length == 3 && data[0] == 0x61 &&
                        ((data[1] == 0xF0 && data[2] == 0x01) ||
                         (data[1] == 0x00 && data[2] == 0x00));
  const bool poll = length == 1 && data[0] == 0x64;
  if (!selector && !poll) {
    publish("INTERNAL READ GUARD BLOCKED BYTES");
    return false;
  }
  usb_transfer_t *transfer = nullptr;
  if (usb_host_transfer_alloc(length, 0, &transfer) != ESP_OK) return false;
...
static void clearProtocolRx() {
  protocolRxTail = protocolRxHead;
}

static bool protocolByte(uint8_t &value, uint32_t timeoutMs) {
  const uint32_t started = millis();
  while (millis() - started < timeoutMs) {
    if (protocolRxTail != protocolRxHead) {
      value = protocolRx[protocolRxTail % sizeof(protocolRx)];
      ++protocolRxTail;
      return true;
    }
    delay(1);
...
static bool readSelector(const uint8_t selector[3]) {
  clearProtocolRx();
  if (!sendReadProtocolBytes(selector, 3)) return false;
  uint8_t response = 0;
  return protocolByte(response, 700) && response == 0x3D;
}

static bool readPolledValues(uint8_t *target, size_t count) {
  const uint8_t poll = 0x64;
  for (size_t index = 0; index < count; ++index) {
    if (!sendReadProtocolBytes(&poll, 1)) return false;
    uint8_t marker = 0;
    uint8_t value = 0;
    if (!protocolByte(marker, 350) || marker != 0x3D || !protocolByte(value, 350)) return false;
    target[index] = value;
  }
  return true;
}

static bool readZeelImage() {
  static const uint8_t headerSelector[3] = {0x61, 0xF0, 0x01};
  static const uint8_t dataSelector[3] = {0x61, 0x00, 0x00};
  if (!ftdiReady || protocolReadBusy) return false;
  protocolReadBusy = true;
  protocolReadActive = true;
  haveZeelImage = false;
  lastReadResult = "reading";
  size_t offset = 0;
  bool ok = readSelector(headerSelector) && readPolledValues(zeelImage + offset, 16);
  offset += ok ? 16 : 0;
  ok = ok && readSelector(headerSelector) && readPolledValues(zeelImage + offset, 16);
  offset += ok ? 16 : 0;
  ok = ok && readSelector(dataSelector) && readPolledValues(zeelImage + offset, 224);
  offset += ok ? 224 : 0;
  ok = ok && readSelector(dataSelector) && readPolledValues(zeelImage + offset, 224);
  offset += ok ? 224 : 0;
  protocolReadActive = false;
  protocolReadBusy = false;
  haveZeelImage = ok && offset == ZEEL_IMAGE_SIZE;
  lastReadResult = haveZeelImage ? "ok" : "protocol_error";
  publish(haveZeelImage ? "STANDALONE READ OK 480 BYTES" : "STANDALONE READ FAILED");
  return haveZeelImage;
}
  ...
  if (transfer->status == USB_TRANSFER_STATUS_COMPLETED) {
    ...
    if (transfer->actual_num_bytes >= 2) {
      ...
      if (payloadLength) {
        ...
        if (protocolReadActive) {
          for (size_t index = 0; index < payloadLength; ++index) {
            protocolRx[protocolRxHead % sizeof(protocolRx)] = payload[index];
            ++protocolRxHead;
            if (protocolRxHead - protocolRxTail > sizeof(protocolRx)) {
              protocolRxTail = protocolRxHead - sizeof(protocolRx);
            }
          }
        }
      }
...
class CfgCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *characteristic) override {
    ...
    if (command.startsWith("BAUD=")) {
      ...
      publish("WRITE ALREADY HARD-LOCKED - READ ONLY");
    } else {
...
static const char PHONE_UI[] PROGMEM = R"HTML(
<!doctype html><html lang="fi"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>MotoLab Zeeltronic</title><style>
body{font-family:system-ui;background:#0d1117;color:#eee;margin:0;padding:20px}main{max-width:620px;margin:auto}
.card{background:#171d25;border:1px solid #2b3440;border-radius:16px;padding:18px;margin:14px 0}
button{width:100%;padding:16px;border:0;border-radius:12px;background:#36c275;color:#07150d;font-size:18px;font-weight:750}
button:disabled{opacity:.45}.lock{color:#ffcb6b}.ok{color:#63e6be}small{color:#9aa7b5}pre{white-space:pre-wrap}
</style><main><h1>MotoLab · Zeeltronic</h1>
<div class="card"><div id="usb">Tarkistetaan FTDI-yhteyttä…</div><small>ESP32-S3 toimii nyt ilman tietokonetta.</small></div>
<div class="card"><button id="read" onclick="readCdi()">Lue CDI</button><p id="result">Ei vielä luettu.</p></div>
<div class="card lock"><b>🔒 Kirjoitus lukittu</b><p>Tämä firmware ei tarjoa Program-toimintoa puhelimelle.</p></div>
<div class="card"><b>Luettu aineisto</b><pre id="data">—</pre></div>
<script>
const usb=document.querySelector('#usb'),btn=document.querySelector('#read'),result=document.querySelector('#result'),data=document.querySelector('#data');
async function status(){try{let s=await (await fetch('/api/status')).json();usb.textContent=s.ftdi_ready?'FTDI 0403:6001 yhdistetty':'FTDI-kaapelia ei havaita';usb.className=s.ftdi_ready?'ok':'';btn.disabled=!s.ftdi_ready||s.busy}catch(e){usb.textContent='Yhteys ESP:hen katkesi'}}
async function readCdi(){btn.disabled=true;result.textContent='Luetaan 480 tavua…';data.textContent='—';try{let r=await fetch('/api/read',{method:'POST'});if(!r.ok)throw Error(await r.text());let b=new Uint8Array(await r.arrayBuffer());result.textContent='Luku onnistui: '+b.length+' tavua';result.className='ok';data.textContent=Array.from(b.slice(0,64),x=>x.toString(16).padStart(2,'0')).join(' ')+' …';let a=document.createElement('a');a.href=URL.createObjectURL(new Blob([b]));a.download='PCDI-10VT-'+new Date().toISOString().replaceAll(':','-')+'.bin';a.textContent='Tallenna varmuuskopio';data.append('\n');data.append(a)}catch(e){result.textContent='Luku epäonnistui: '+e.message;result.className=''}finally{status()}}
status();setInterval(status,2500);
</script></main></html>)HTML";

static void startStandaloneWeb() {
  WiFi.mode(WIFI_AP);
  WiFi.softAP(WIFI_NAME, WIFI_PASSWORD);
  web.on("/", HTTP_GET, []() { web.send_P(200, "text/html; charset=utf-8", PHONE_UI); });
  web.on("/api/status", HTTP_GET, []() {
    const String json = String("{\"fw\":\"") + FW + "\",\"ip\":\"" +
                        WiFi.softAPIP().toString() + "\",\"ftdi_ready\":" +
                        (ftdiReady ? "true" : "false") + ",\"busy\":" +
                        (protocolReadBusy ? "true" : "false") + ",\"write_enabled\":false," +
                        "\"last_read\":\"" + lastReadResult + "\"}";
    web.send(200, "application/json", json);
  });
  web.on("/api/read", HTTP_POST, []() {
    if (!ftdiReady) { web.send(503, "text/plain", "FTDI 0403:6001 ei ole yhteydessä"); return; }
    if (protocolReadBusy) { web.send(409, "text/plain", "Luku on jo käynnissä"); return; }
    if (!readZeelImage()) { web.send(502, "text/plain", "Zeeltronic-protokollan vastaus ei täsmännyt"); return; }
    web.sendHeader("Content-Disposition", "attachment; filename=PCDI-10VT.bin");
    web.setContentLength(ZEEL_IMAGE_SIZE);
    web.send(200, "application/octet-stream", "");
    web.client().write(zeelImage, ZEEL_IMAGE_SIZE);
  });
  web.onNotFound([]() { web.sendHeader("Location", "/"); web.send(302); });
  web.begin();
  publish(String("WIFI READY ssid=") + WIFI_NAME + " ip=" + WiFi.softAPIP().toString());
}
...
void loop() {
  ...
  if (ftdiReady && millis() - lastHeartbeat >= 5000) {
    lastHeartbeat = millis();
    publish(String("HEARTBEAT ") + statsLine());
  }
}
