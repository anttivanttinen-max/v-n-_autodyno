...
// MotoLab / Zeeltronic ESP32-S3 bridge — stage 2.1 FTDI transport
// Target: ESP32-S3 N16R8 + Arduino-ESP32 3.3.11.
// PC/debug: board Single Serial UART bridge. Native USB-OTG: Zeeltronic FTDI 0403:6001.
// Transparent BLE byte pipe. This firmware does NOT alter CDI settings on its own.

static const char *FW = "motolab-zeel-ftdi-bridge-v2.1";
...
static void publish(const String &msg){
  Serial0.println(msg);
  if(statusChar){ statusChar->setValue(msg); statusChar->notify(); }
}

struct CtrlWait { volatile bool done; usb_transfer_status_t status; };
static void controlDone(usb_transfer_t *t){
  CtrlWait *w=(CtrlWait*)t->context;
  if(w){ w->status=t->status; w->done=true; }
}
...
static esp_err_t controlOut(uint8_t req,uint16_t value,uint16_t index=0){
  ...
  esp_err_t e=usb_host_transfer_alloc(sizeof(usb_setup_packet_t),0,&t);
  ...
  CtrlWait wait{false,USB_TRANSFER_STATUS_ERROR};
  t->device_handle=deviceHandle;
  t->bEndpointAddress=0;
  t->callback=controlDone;
  t->context=&wait;
  ...
  if(e==ESP_OK){
    ...
    while(!wait.done && millis()-start<1000){
      usb_host_client_handle_events(clientHandle,pdMS_TO_TICKS(10));
    }
    if(!wait.done)e=ESP_ERR_TIMEOUT;
    else if(wait.status!=USB_TRANSFER_STATUS_COMPLETED)e=ESP_FAIL;
  }
...
static bool configureFtdi(uint32_t baud){
  ...
  uint16_t v=0,i=0;
  if(!ftdiBaudValue(baud,v,i)){publish("UNSUPPORTED BAUD");return false;}
  if(controlOut(0,0,interfaceNumber+1)!=ESP_OK)return false;
  if(controlOut(4,0x0008,interfaceNumber+1)!=ESP_OK)return false;
  if(controlOut(3,v,i)!=ESP_OK)return false;
  if(controlOut(2,0x0000,interfaceNumber+1)!=ESP_OK)return false;
  controlOut(1,0x0303,interfaceNumber+1);
  baudRate=baud;
  publish(String("FTDI CONFIGURED ")+baudRate+" 8N1");
  if(cfgChar)cfgChar->setValue(String("BAUD=")+baudRate);
  return true;
}

static bool findInterfaceAndBulkEndpoints(){
  ...
  while(p+2<=end){
    ...
    if(type==USB_B_DESCRIPTOR_TYPE_INTERFACE && len>=9){
      ...
      interfaceNumber=itf->bInterfaceNumber;
      inTargetInterface=true;
    }else if(type==USB_B_DESCRIPTOR_TYPE_ENDPOINT && len>=7 && inTargetInterface){
...
static void rxDone(usb_transfer_t *t){
  if(t->status==USB_TRANSFER_STATUS_COMPLETED && t->actual_num_bytes>2){
    const uint8_t *d=t->data_buffer+2;
    size_t n=t->actual_num_bytes-2;
    Serial0.write(d,n);
    if(rxChar){rxChar->setValue(d,n);rxChar->notify();}
  }
  if(ftdiReady&&deviceHandle){
    t->num_bytes=t->data_buffer_size;
    if(usb_host_transfer_submit(t)!=ESP_OK)usb_host_transfer_free(t);
  }else usb_host_transfer_free(t);
}

static bool startRx(){
  usb_transfer_t *t=nullptr;
  size_t n=epInMps>64?epInMps:64;
  if(usb_host_transfer_alloc(n,0,&t)!=ESP_OK)return false;
  t->device_handle=deviceHandle;
  t->bEndpointAddress=epIn;
  t->callback=rxDone;
  t->context=nullptr;
  t->num_bytes=n;
...
static void usbDaemonTask(void*){
  ...
  esp_err_t e=usb_host_install(&c);
  if(e!=ESP_OK){publish(String("USB HOST INSTALL ERROR ")+esp_err_to_name(e));vTaskDelete(nullptr);return;}
  publish("USB HOST READY");
  while(true){
    uint32_t f=0;
    usb_host_lib_handle_events(pdMS_TO_TICKS(1000),&f);
...
static void closeDevice(){
  ftdiReady=false;
  if(deviceHandle && interfaceClaimed){
    usb_host_endpoint_halt(deviceHandle,epIn);
    usb_host_endpoint_halt(deviceHandle,epOut);
    usb_host_endpoint_flush(deviceHandle,epIn);
    usb_host_endpoint_flush(deviceHandle,epOut);
...
static void usbClientTask(void*){
  ...
  esp_err_t e=usb_host_client_register(&c,&clientHandle);
  if(e!=ESP_OK){publish(String("USB CLIENT ERROR ")+esp_err_to_name(e));vTaskDelete(nullptr);return;}
  publish("USB CLIENT READY — connect Zeeltronic PC-USB cable");
  while(true){
    usb_host_client_handle_events(clientHandle,pdMS_TO_TICKS(100));
    if(deviceGone&&deviceHandle){closeDevice();deviceGone=false;}
    uint8_t a=pendingAddress;
    if(a&&!deviceHandle){
      pendingAddress=0;
      e=usb_host_device_open(clientHandle,a,&deviceHandle);
      if(e!=ESP_OK){publish(String("USB OPEN ERROR ")+esp_err_to_name(e));continue;}
      const usb_device_desc_t *d=nullptr;
      e=usb_host_get_device_descriptor(deviceHandle,&d);
      if(e!=ESP_OK||!d){publish("USB DESCRIPTOR ERROR");closeDevice();continue;}
      char line[128];
      snprintf(line,sizeof(line),"USB VID=%04X PID=%04X class=%02X",d->idVendor,d->idProduct,d->bDeviceClass);publish(line);
      if(d->idVendor!=0x0403||d->idProduct!=0x6001){publish("NOT ZEEL FTDI 0403:6001");closeDevice();continue;}
      if(!findInterfaceAndBulkEndpoints()){publish("FTDI BULK ENDPOINTS NOT FOUND");closeDevice();continue;}
      e=usb_host_interface_claim(clientHandle,deviceHandle,interfaceNumber,0);
      if(e!=ESP_OK){publish(String("FTDI INTERFACE CLAIM ERROR ")+esp_err_to_name(e));closeDevice();continue;}
      interfaceClaimed=true;
      snprintf(line,sizeof(line),"FTDI READY if=%u epIN=0x%02X epOUT=0x%02X",interfaceNumber,epIn,epOut);publish(line);
      ftdiReady=true;
      if(!configureFtdi(baudRate))publish("FTDI CONFIG ERROR");
...
void setup(){
  ...
  svc->start();
  BLEAdvertising *a=BLEDevice::getAdvertising();a->addServiceUUID(SERVICE_UUID);a->setScanResponse(true);BLEDevice::startAdvertising();
