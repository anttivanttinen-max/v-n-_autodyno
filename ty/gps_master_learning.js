...
function referenceText(){
  const r=gpsReferenceMeta();
  if(r.ratio)return `${r.source==='profile_drivetrain'?'VÄLITYKSISTÄ':'GPS-KALIBROINNISTA'} ${r.ratio.toFixed(2)} rpm/kmh`;
  return 'RPM-REFERENSSI PUUTTUU: kalibroi vaihde tai anna välitykset + renkaan ympärys';
}
...
function installControls(){
  ...
  $m('gpsMasterPrepare').onclick=async()=>{
    ...
    if(typeof addLearningEvent==='function')addLearningEvent('gps_master_mic_learn_prepare',{gpsOk:!!gpsOk,micOk:!!micOk,selectedGear:ref.gear,module:MODULE_VERSION,referenceSource:ref.source,rpmPerKmh:ref.ratio});
...
function patchCore(){
  if(typeof currentSettings==='function'&&!currentSettings.__gpsMasterPatched){
    ...
    const patched=function(){
      ...
      if($m('rpmSourceMode')?.value===MODE){
        ...
        const ref=drivetrainReference();
        if(ref){
          const existing=(c.gpsCalibrations||[]).filter(x=>+x.gear!==+ref.gear);
          c.gpsCalibrations=[...existing,{gear:ref.gear,ratio:ref.ratio,source:'profile_drivetrain'}];
          c.selectedGear=ref.gear;
        }
      }
      return c;
    };
  ...
  if(typeof updateSourceModeUI==='function'&&!updateSourceModeUI.__gpsMasterPatched){
    ...
    const patched=function(){
      ...
      if(s)s.textContent=`GPS MASTER + MIC LEARN: GPS/nopeus + valittu vaihde määrää RPM:n (${referenceText()}). BT MIC tallennetaan shadow-dataksi eikä se vaikuta RPM:ään, vaihdeoppimiseen tai vedon hyväksyntään.`;
  ...
  if(typeof collectLearningSample==='function'&&!collectLearningSample.__gpsMasterPatched){
    const patched=function(s){
      ...
      const refMeta=gpsReferenceMeta();
      ...
      const row={
        ...
        audioRawRpm:raw,audioRpm:mic,audioConf:Number.isFinite(s.audioConf)?s.audioConf:null,observedF0Hz:Number.isFinite(s.audioObservedF0)?s.audioObservedF0:null,
        fingerprintScore:Number.isFinite(s.audioFpScore)?s.audioFpScore:null,candidateGap:Number.isFinite(s.audioCandidateGap)?s.audioCandidateGap:null,runnerRpm:Number.isFinite(s.audioRunnerRpm)?s.audioRunnerRpm:null,audioLevel:Number.isFinite(s.audioLevel)?s.audioLevel:null,
        speedRpm:gpsRef,speedConf:gpsRefConf,fusedRpm:Number.isFinite(s.fusionRpm)?s.fusionRpm:null,fusedConf:Number.isFinite(s.conf)?s.conf:null,
        source:s.source||null,fusionMode:s.fusionMode||null,gpsKmh:Number.isFinite(s.kmh)?s.kmh:null,gpsAccuracyM:Number.isFinite(s.gpsAcc)?s.gpsAcc:null,
        accelerationMps2:Number.isFinite(s.a)?s.a:null,accelerationG:Number.isFinite(s.g)?s.g:null,motionMag:Number.isFinite(s.motionMag)?s.motionMag:null,motionAgeMs:Number.isFinite(s.motionAgeMs)?s.motionAgeMs:null,
        gear:s.gear||null,gearConf:Number.isFinite(s.gearConf)?s.gearConf:null,rpmPerKmh:Number.isFinite(s.ratio)?s.ratio:null,slip:!!s.slip,agreement:Number.isFinite(s.agreement)?s.agreement:null,
        estimatedHp:Number.isFinite(s.hp)?s.hp:null,estimatedNm:Number.isFinite(s.nm)?s.nm:null,sampleQuality:Number.isFinite(s.quality)?s.quality:null
      };
...
function patchLiveStatus(){
  ...
  sel.addEventListener('change',()=>{
    if(sel.value===MODE&&typeof addLearningEvent==='function')addLearningEvent('gps_master_mic_learn_selected',{selectedGear:+($m('manualGear')?.value||0)||null,module:MODULE_VERSION,reference:gpsReferenceMeta()});
    const st=$m('gpsMasterState'); if(st)st.textContent=sel.value===MODE?`GPS MASTER valittu • ${referenceText()}`:'GPS MASTER ei valittuna.';
  });
  $m('manualGear')?.addEventListener('change',()=>{if(sel.value===MODE){if(typeof updateSourceModeUI==='function')updateSourceModeUI();const st=$m('gpsMasterState');if(st)st.textContent=`Vaihde vaihdettu • ${referenceText()}`;}});
}
...
function boot(){
  ...
  if(typeof addLearningEvent==='function')addLearningEvent('gps_master_module_loaded',{module:MODULE_VERSION,reference:gpsReferenceMeta()});
}
