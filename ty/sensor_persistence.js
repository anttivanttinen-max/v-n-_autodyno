...
let reconnectFailures=0,nextReconnectAt=0,lastRecoveryReason='';
...
async function recoverMic(reason,force=false){
 ...
 const now=Date.now();if(!force&&now<nextReconnectAt)return false;
 ...
 try{
  ...
  let ok=false,startError='';
  try{if(typeof startAudio==='function')ok=await startAudio()}catch(e){startError=e?.name||e?.message||String(e)}
  await new Promise(r=>setTimeout(r,180));
  const live=!!ok&&micActive(),newTrack=micTrack();
  if(live){
   ...
   logRecovery('mic_recovery_success',{reason,attempt});
  }else{
   ...
   logRecovery('mic_recovery_failure',{ok:false,reason,attempt,retryInMs,error:startError||'track_not_live',newTrackState:newTrack?.readyState||'none'});
   if(reconnectFailures>=3)showRecoveryAction();
  }
