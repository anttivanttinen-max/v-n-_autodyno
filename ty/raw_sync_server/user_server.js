...
function hasAdmin(db){return db.users.some(x=>x.role==='admin')}
function promoteOwner(db,u,deviceId,source='owner_bootstrap'){
 if(!u)return null;
 u.nickname=OWNER_NICKNAME;u.status='active';u.role='admin';u.approvedAt=u.approvedAt||new Date().toISOString();u.ownerClaimedAt=new Date().toISOString();u.ownerClaimedDeviceId=deviceId;u.ownerClaimSource=source;writeUsers(db);return u
}
...
async function claimOwner(req,res,origin){const x=requireIdentity(req,res,origin);if(!x)return;let data;try{data=JSON.parse((await readBody(req)).toString('utf8'))}catch{return send(res,400,{ok:false,error:'Invalid JSON'},origin)}const code=String(data.code||data.invite||'').trim();if(staticInviteIndex(code)!==0)return send(res,403,{ok:false,error:'Owner code invalid'},origin);const db=readUsers(),u=findUser(db,x.user.userId);const existing=db.users.find(v=>v.role==='admin'&&v.userId!==u.userId);if(existing)return send(res,409,{ok:false,error:'Owner already exists'},origin);promoteOwner(db,u,x.device.deviceId,'owner_claim_endpoint');const t=tokenFor(u,x.device.deviceId);return send(res,200,{ok:true,...t,user:publicUser(u),owner:true},origin)}
...
http.createServer=function(listener){return originalCreateServer(async(req,res)=>{const origin=String(req.headers.origin||'');let u;try{u=new URL(req.url,'http://localhost')}catch{return listener(req,res)}
 ...
 if(req.method==='POST'&&u.pathname==='/api/users/v1/claim-owner')return claimOwner(req,res,origin);
...
console.log(`MotoLab user server: ${BETA_SECRET?'enabled':'not configured'}; staticInvites=${STATIC_INVITES.length}; adminDevices=${ADMIN_DEVICE_IDS.length}; ownerNickname=${OWNER_NICKNAME}`);
